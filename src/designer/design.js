...
// 1. 시스템 프롬프트 (LLM이 채울 슬롯 A·B·C 규칙)
...
// 2-2. 카테고리 게이트 — 시안가는 5대분류(클렌징·스킨케어·메이크업>립/베이스/아이)만 지원
if (!isCategorySupported(brandCategory)) {
  ...
  writeFileSync(
    ...
    JSON.stringify(
      wrapError(`시안가 미지원 카테고리: ${brandCategory}`),
      null,
      2,
    ),
    "utf-8",
...
// 4. 사용자 메시지 — 작성가 콘텐츠 + 브랜드 카테고리(LLM은 A/B/C 슬롯만 채움)
const userMessage = `다음 작성가 콘텐츠 기획으로 시안 슬롯(A/B/C)을 채우세요.
...
- person.a_model, b_product, c_background, negative_prompt, aspect_ratio, concept, visual_direction (모두 작성)
- product.b_product, c_background, negative_prompt, aspect_ratio, concept, visual_direction (a_model은 비움 — 제품 샷은 코드 매트릭스가 a_product를 채움)
- [D 자세 및 구도]는 절대 만들지 마세요. 코드가 룩업으로 채웁니다.
- 고정 틀·8k wallpaper 마무리도 코드가 조립합니다. LLM은 슬롯 값만 출력.`;
...
function buildPersonPrompt({ a_model, b_product, d_pose, c_background }) {
  return `1girl, korean beauty influencer, ${a_model}, ${b_product}, ${d_pose}, ${c_background}, high-fashion cosmetic advertising composition, directional beauty lighting, skin sheen, soft catchlights in the eyes, sharp focus, shallow depth of field, creamy bokeh, cinematic lighting, photorealistic, hyper-detailed, masterpiece, best quality, 8k wallpaper`;
}
function buildProductPrompt({ a_product, b_product, d_layout, c_background }) {
  return `product still life photography, no human, no model, ${a_product}, ${b_product}, ${d_layout}, ${c_background}, premium cosmetic advertising composition, commercial product shot, sharp focus, hyper-detailed product texture, soft reflections, photorealistic, masterpiece, best quality, 8k wallpaper`;
}
  ...
  if (personD) {
    visuals.push({
      ...
      generation_prompt: buildPersonPrompt({
        a_model: c.person.a_model ?? "",
        b_product: c.person.b_product,
        d_pose: personD,
