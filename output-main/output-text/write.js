...
export function generateReport({ brand, trend, match, trendSummary } = {}) {
  ...
  // Part II (트렌드 카드 — 유행현황·수집근거·매칭이유): 항상 그대로 노출.
...
// Part II 렌더 — 트렌드 카드 3장 (키워드·유행현황·수집근거·매칭이유).
...
//
// 유사도 가드: Part II 유행현황(summary_bullets) 텍스트와 문자 bigram Jaccard
// 유사도가 50% 이상이면 LLM이 유행현황을 그대로 베껴 쓴 거라 보고 재시도.
...
async function buildTrendSummary({ contents, client }) {
  ...
  // Part II 유행현황 텍스트 — summary_bullets 통째. 유사도 비교 기준.
  ...
  const baseSystem = [
    ...
    "- **유행현황 텍스트를 그대로 옮겨 적지 말고**, 세 트렌드를 가로지르는 거시 흐름·시사점을 새로운 표현으로 추출",
    "- 개별 사실 나열보다 '왜 지금 이런 흐름이 동시에 일어나는가'라는 메타 관점을 살릴 것",
    "- 따옴표·번호·줄바꿈·다른 설명 없이 문장만 출력",
  ].join("\n");

  // 임계 10% — 유행현황과 표현 겹침을 거의 허용 안 함. 한국어 bigram 특성상
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    const stricter =
      attempt > 0
        ? `\n\n(이전 시도가 유행현황과 ${(lastSim * 100).toFixed(0)}% 유사했음. 임계 ${(SIMILARITY_LIMIT * 100).toFixed(0)}% 미만 필요. 원문 어휘를 거의 쓰지 말고, 추상화된 거시 흐름·시사점 위주로 표현을 새로 짤 것. 동의어·상위 개념으로 치환.)`
        : "";
    ...
    try {
      ...
      console.warn(
        `⚠️ buildTrendSummary 시도 ${attempt + 1}/${MAX_ATTEMPTS}: 유행현황과 유사도 ${(sim * 100).toFixed(1)}% — 재시도 (임계 ${(SIMILARITY_LIMIT * 100).toFixed(0)}%)`,
      );
    } catch (err) {
      console.warn(
...
// 각 카드의 summary_bullets(=Part II 유행현황)를 한 문장으로 압축해 PART I
...
async function buildCardIntroSummary(card, client) {
  ...
  const baseSystem = [
    "다음 한 뷰티 트렌드의 유행현황을 마케터용 리포트 PART I 카드 본문에 들어갈 자연스러운 한국어 한 문장으로 요약해줘.",
    "- 마침표 1개, 한 문장만",
    "- 유행현황 원문의 어휘를 그대로 옮기지 말고, 상위 개념·동의어·다른 시각으로 표현 새로 짤 것",
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    const stricter =
      attempt > 0
        ? `\n\n(이전 시도가 유행현황과 ${(lastSim * 100).toFixed(0)}% 유사했음. 임계 10% 미만 필요. 원문 단어 거의 피하고 더 상위 추상화·다른 각도로 표현.)`
        : "";
    try {
      const response = await client.messages.create({
        model: "claude-haiku-4-5",
...
function buildMatchReason(fitData, td, fitIndex = 0) {
  ...
  const polishedFinal = /[.。!?]$/.test(polished) ? polished : `${polished}.`;
...
export async function generateWriterOutput({ brand, trend, match, trendRaw } = {}) {
  ...
  // 4) 카드별 PART I 인트로 요약 — 유행현황과 똑같은 텍스트 반복 방지.
  ...
  // 그대로 유지 (Part II 유행현황·수집근거·매칭이유 렌더링에 필요).
