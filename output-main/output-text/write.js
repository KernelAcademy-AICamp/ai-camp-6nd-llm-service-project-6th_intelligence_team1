...
export function generateReport({ brand, trend, match, trendSummary } = {}) {
  ...
  // Part I (트렌드 요약): trendSummary 있으면 한 문장으로 대체, 없으면 A/B/C 브리프.
  if (trendSummary) {
    lines.push("## 📌 트렌드 요약");
    lines.push("");
    ...
    renderPartI(lines, top, findTrend);
  }

  // Part II (트렌드 카드 — 트렌드현황·수집근거·매칭이유): 항상 그대로 노출.
  renderPartII(lines, top, findTrend);
  ...
  lines.push("| 한 줄 요약 · 의미 · 트렌드현황 | `trend-analysis.json` | 이효희 (분석) |");
...
// Part I 렌더 — A/B/C 트렌드 한 줄 브리프. trendSummary 없을 때 fallback.
function renderPartI(lines, top, findTrend) {
...
// Part II 렌더 — 트렌드 카드 3장 (키워드·트렌드현황·수집근거·매칭이유).
// trendSummary 유무와 무관하게 항상 노출.
function renderPartII(lines, top, findTrend) {
  ...
  top.forEach((r, i) => {
    ...
    // 📊 트렌드현황 — 의미(meaning)를 첫 불렛으로 흡수 (별도 의미 블록 제거)
    lines.push("**📊 트렌드현황 (Status)**");
    const statusBullets = [];
...
function buildSourceName(td) {
  ...
  const others = [];
  sources.forEach((s) => {
    const label = SOURCE_KO[s] ?? s;
    if (label.startsWith("네이버 ")) naverKinds.push(label.replace("네이버 ", ""));
    else if (label === "네이버") naverKinds.push("");
    else others.push(label);
  });
  const naverPart = naverKinds.length
...
// score는 LLM 추정값이라 숫자(지수) 빼고 tier 라벨만 표기. growth_rate는 노출 유지.
// 출처 attribution(예: "네이버 뉴스 기준")을 앞에 붙여 어디서 분석된 수치인지 명시.
function buildIndexPhrase(td) {
  const score = td?.metrics?.score;
  if (typeof score !== "number") return null;
  const tier =
    score >= 80 ? "상위권" : score >= 60 ? "중상위권" : score >= 40 ? "중간" : "하위권";
  const source = buildSourceAttribution(td);
  const rate = td?.metrics?.growth_rate;
  if (typeof rate === "number" && Math.round(rate * 100) !== 0) {
    const pct = Math.round(rate * 100);
    const sign = pct > 0 ? "+" : "";
    return `${source}검색 관심도 ${tier} (${sign}${pct}%)`;
  }
  return `${source}검색 관심도 ${tier}`;
}

// summary_bullets 끝에 붙는 통계 한 줄 요약. 트렌드 레벨 수치를 한 곳에 모아
// 카드별 stat 반복을 피한다. 출처가 있는 값(검색량)에만 출처 라벨을 단다.
//   "트렌드 분석 기준 검색 관심도 상위권 (+20%) · 네이버 검색광고 기준 월간 검색량 1,890건"
// 데이터가 없으면 해당 segment 생략. 둘 다 없으면 null.
function buildStatsSummaryLine(td) {
  const segments = [];

  // 1) 검색 관심도(tier) + 증가율 — LLM 추정값. tier 라벨만 표기, 숫자 없음.
  // 출처: evidence의 실제 수집 채널(예: "네이버 뉴스·블로그 기준")을 노출해
  //       "트렌드 분석 기준"이라는 모호한 표현 대신 구체적인 데이터 채널을 명시.
  const score = td?.metrics?.score;
  if (typeof score === "number") {
    const tier =
      score >= 80 ? "상위권" : score >= 60 ? "중상위권" : score >= 40 ? "중간" : "하위권";
    const sourceLabel = buildSourceAttribution(td) || "트렌드 분석 기준 ";
    const rate = td?.metrics?.growth_rate;
    if (typeof rate === "number" && Math.round(rate * 100) !== 0) {
      const pct = Math.round(rate * 100);
      const sign = pct > 0 ? "+" : "";
      segments.push(`${sourceLabel}검색 관심도 ${tier} (${sign}${pct}%)`);
    } else {
      segments.push(`${sourceLabel}검색 관심도 ${tier}`);
    }
  }

  // 2) 월간 검색량 — 네이버 검색광고 API 실측값.
  const df = td?.demand_fit;
  if (df && df.invalid_keyword !== true) {
    const ms = df.monthly_searches;
    if (typeof ms === "number" && ms > 0) {
      segments.push(`네이버 검색광고 기준 월간 검색량 ${ms.toLocaleString("ko-KR")}건`);
    }
  }
...
// 출처 라벨(네이버 검색광고)을 함께 노출 — 실측 데이터임을 마케터가 즉시 인지.
function buildSearchVolumePhrase(td) {
  const df = td?.demand_fit;
  if (!df || df.invalid_keyword === true) return null;
  const ms = df.monthly_searches;
  if (typeof ms !== "number" || ms <= 0) return null;
  return `네이버 검색광고 기준 월간 검색량 ${ms.toLocaleString("ko-KR")}건`;
}
...
// 트렌드 진입 전략 한 문장 — stage·growth·lifespan 조합으로 마케터에게
// "그래서 어떻게 진입할지" 가이드. PART I 트렌드 요약과 겹치지 않도록 raw 수치
// 대신 전략적 해석에 집중.
//   emerging + 급등: "급등하는 단기 트렌드라, 지금 기획에 빠르게 넣으면 ..."
//   emerging + 보통: "아직 검색량이 폭발하기 전인 성장 초입(N개월 흐름)이라, ..."
//   peak + 장기:     "급등이 아니라 꾸준한 우상향(N개월)이라, 리스크 부담 없이 ..."
//   peak + 단기:     "정점에 도달한 트렌드라, 빠른 집행이 효과적이에요."
//   declining:       "하락세에 진입한 트렌드라, 한정적인 회수 시점에만 ..."
function buildStrategicSentence(td) {
  const stage = td?.trend_stage;
  const rate = td?.metrics?.growth_rate ?? 0;
  const lifespan = String(td?.lifespan_estimate ?? "").trim();
  const absPct = Math.abs(Math.round(rate * 100));
  const isHighGrowth = absPct >= 50;
  const isLong = /6개월|이상/.test(lifespan);
  const lifeTag = lifespan ? `(${lifespan} 흐름)` : "";

  // 종결 마침표는 buildUsagePlan 최종 join에서 일괄 부착하므로 여기선 생략.
  // 안정 case: lifeTag에 "흐름" 안 붙이고 콤마 없이 자연스럽게 흘림.
  // 초입 case: lifeTag에 "흐름" 붙이고 "...진입할 수 있는 트렌드예요"로 종결.
  const lifeTagPlain = lifespan ? `(${lifespan})` : "";
  const lifeTagFlow = lifespan ? `(${lifespan} 흐름)` : "";

  if (stage === "declining") {
    return "하락세에 진입한 트렌드라, 한정적인 회수 시점에만 집행하는 게 좋아요";
  }
  if (stage === "emerging" && isHighGrowth) {
    return "급등하는 단기 트렌드라, 지금 기획에 빠르게 넣으면 경쟁사보다 먼저 진입할 수 있는 트렌드예요";
  }
  if (stage === "emerging") {
    return `아직 검색량이 폭발하기 전인 성장 초입${lifeTagFlow}이라, 지금 기획에 넣으면 경쟁사보다 먼저 진입할 수 있는 트렌드예요`;
  }
  if (stage === "peak" && isLong) {
    return `급등이 아니라 꾸준한 우상향${lifeTagPlain}이라 리스크 부담 없이 안정적으로 진입할 수 있어요`;
  }
  if (stage === "peak") {
    return "정점에 도달한 트렌드라, 빠른 집행이 효과적이에요";
  }
  return "안정적인 흐름으로 진입할 수 있는 트렌드예요";
}
...
function buildUsagePlan(brand, td, opts = {}) {
  ...
  if (items.length > 0) {
    ...
    if (matched) {
      ...
      // 매체별 활성도 — 마케터 채널 매칭 안 되고 트렌드 채널을 안내하는 경우.
      channelTag = `${channel}에서 활성중인 트렌드예요.`;
  ...
  // ─ 문장 1: 매체 활성도 + 전략 해석 ─────────────────────────────
  // PART I 트렌드 요약(출처·수치·기간)과 중복되던 raw 데이터(evidence·지수·검색량
  // ·헤드라인) 제거. 활용방안은 마케터가 "그래서 어떻게 진입할지" 판단할 수
  // 있는 전략 문장으로 좁힘.
  //   1) 매체 활성도 — 마케터 채널과 트렌드 채널 매칭 여부 한 줄
  //   2) 전략 해석 — stage·growth·lifespan 조합으로 진입 전략 한 줄
  const strategicSentence = buildStrategicSentence(td);
  const parts1 = [channelTag, strategicSentence].filter(Boolean);
  const sentence1 = parts1.length > 0 ? parts1.join(" ") : null;
...
// ─── 트렌드 한 문장 요약 (LLM 호출) ─────────────────────────────────
// contents 카드들의 trend_name + summary_bullets를 종합해 마케터용 리포트의
// 한국어 한 문장으로 압축. client가 null이거나 호출 실패 시 null 반환 →
// 호출부에서 카드 그대로 두는 fallback 진행.
//
// 유사도 가드: Part II 트렌드현황(summary_bullets) 텍스트와 문자 bigram Jaccard
// 유사도가 50% 이상이면 LLM이 트렌드현황을 그대로 베껴 쓴 거라 보고 재시도.
// 3회 시도 모두 임계 초과면 null 반환.

// 한글 문자 bigram Jaccard 유사도. 공백·구두점 제거 후 인접 두 글자 set 비교.
// 짧은 한국어 텍스트에서 "표현이 거의 같은가"를 가늠하기 좋은 가벼운 지표.
...
async function buildTrendSummary({ contents, client }) {
  if (!client) return null;
  if (!Array.isArray(contents) || contents.length === 0) return null;

  // evidence의 label(예: "Naver News")을 사람이 읽는 라벨로 노출. LLM이 수치를
  // 인용할 때 그 출처를 같이 언급할 수 있도록 digest에 함께 묶는다.
  ...
  const digest = contents
    .map((c, i) => {
      const bullets = (c.summary_bullets ?? []).filter(Boolean).join(" / ");
      const evidenceList = (c.evidence ?? [])
        .map((e) => `${sourceLabel(e.source)}: ${e.description ?? e.metric ?? ""}`.trim())
        .filter(Boolean);
      const evidenceBlock = evidenceList.length
        ? `\n수집 근거:\n  - ${evidenceList.join("\n  - ")}`
        : "";
      return `${i + 1}. ${c.trend_name}\n${bullets}${evidenceBlock}`;
    })
    .join("\n\n");

  // Part II 트렌드현황 텍스트 — summary_bullets 통째. 유사도 비교 기준.
  const statusText = contents
    .map((c) => (c.summary_bullets ?? []).filter(Boolean).join(" "))
    .join(" ");

  const baseSystem = [
    "다음 뷰티 트렌드들을 종합해서 마케터용 리포트에 들어갈 자연스러운 한국어 딱 한 문장으로 요약해줘.",
    "- 트렌드별로 나누지 말고 전체를 묶어서 하나의 문장(마침표 1개)으로 만들 것",
    "- 수치를 인용할 때는 어느 출처(예: 네이버 뉴스, 네이버 블로그, 유튜브)인지 문장 안에 자연스럽게 포함",
    "- **트렌드현황 텍스트를 그대로 옮겨 적지 말고**, 세 트렌드를 가로지르는 거시 흐름·시사점을 새로운 표현으로 추출",
    "- 개별 사실 나열보다 '왜 지금 이런 흐름이 동시에 일어나는가'라는 메타 관점을 살릴 것",
    "- 따옴표·번호·줄바꿈·다른 설명 없이 문장만 출력",
  ].join("\n");

  // 임계 10% — 트렌드현황과 표현 겹침을 거의 허용 안 함. 한국어 bigram 특성상
  // 같은 도메인(뷰티) 어휘만으로도 6~10% 겹침이 자연스럽게 발생해 빡빡한 기준.
  // 도달 못 하면 fallback이 동작하도록 시도 횟수와 강도 조정.
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    const stricter =
      attempt > 0
        ? `\n\n(이전 시도가 트렌드현황과 ${(lastSim * 100).toFixed(0)}% 유사했음. 임계 ${(SIMILARITY_LIMIT * 100).toFixed(0)}% 미만 필요. 원문 어휘를 거의 쓰지 말고, 추상화된 거시 흐름·시사점 위주로 표현을 새로 짤 것. 동의어·상위 개념으로 치환.)`
        : "";
    ...
    try {
      const response = await client.messages.create({
        model: "claude-haiku-4-5",
        max_tokens: 320,
        temperature: Math.min(0.5 + attempt * 0.15, 1.0),
        system: baseSystem + stricter,
      ...
      const sim = bigramJaccard(text, statusText);
      lastSim = sim;
      if (sim < SIMILARITY_LIMIT) {
        return text;
      }
      console.warn(
        `⚠️ buildTrendSummary 시도 ${attempt + 1}/${MAX_ATTEMPTS}: 트렌드현황과 유사도 ${(sim * 100).toFixed(1)}% — 재시도 (임계 ${(SIMILARITY_LIMIT * 100).toFixed(0)}%)`,
      );
    } catch (err) {
      console.warn(
        `⚠️ buildTrendSummary 시도 ${attempt + 1}/${MAX_ATTEMPTS} 실패: ${err.message}`,
      );
    }
  }

  console.warn(
    `⚠️ buildTrendSummary: ${MAX_ATTEMPTS}회 시도 모두 유사도 임계(${(SIMILARITY_LIMIT * 100).toFixed(0)}%) 초과 — fallback (요약 미생성)`,
  );
  return null;
}

// ─── 카드별 PART I 인트로 요약 (LLM 호출) ───────────────────────────
// 각 카드의 summary_bullets(=Part II 트렌드현황)를 한 문장으로 압축해 PART I
// 본문에 노출. 같은 텍스트가 반복되지 않도록 유사도 < 10% 가드 + 5회 재시도.
// null 반환 시 호출부에서 summary_bullets[0] fallback.
async function buildCardIntroSummary(card, client) {
  if (!client) return null;
  const bullets = (card?.summary_bullets ?? []).filter(Boolean);
  if (bullets.length === 0) return null;

  const statusText = bullets.join(" ");
  const digest = `트렌드명: ${card.trend_name ?? "?"}\n트렌드현황:\n- ${bullets.join("\n- ")}`;

  const baseSystem = [
    "다음 한 뷰티 트렌드의 트렌드현황을 마케터용 리포트 PART I 카드 본문에 들어갈 자연스러운 한국어 한 문장으로 요약해줘.",
    "- 마침표 1개, 한 문장만",
    "- 트렌드현황 원문의 어휘를 그대로 옮기지 말고, 상위 개념·동의어·다른 시각으로 표현 새로 짤 것",
    "- 핵심 가치 한 가지만 짧게 짚고 세부 수치·고유명사·인용은 생략",
    "- 따옴표·번호·줄바꿈·다른 설명 없이 문장만 출력",
  ].join("\n");
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    const stricter =
      attempt > 0
        ? `\n\n(이전 시도가 트렌드현황과 ${(lastSim * 100).toFixed(0)}% 유사했음. 임계 10% 미만 필요. 원문 단어 거의 피하고 더 상위 추상화·다른 각도로 표현.)`
        : "";
    try {
      const response = await client.messages.create({
        model: "claude-haiku-4-5",
      ...
      const sim = bigramJaccard(text, statusText);
      lastSim = sim;
      if (sim < SIMILARITY_LIMIT) return text;
      console.warn(
        `⚠️ buildCardIntroSummary[${(card.trend_name ?? "?").slice(0, 25)}] 시도 ${attempt + 1}/${MAX_ATTEMPTS}: 유사도 ${(sim * 100).toFixed(1)}%`,
      );
    } catch (err) {
      console.warn(
        `⚠️ buildCardIntroSummary 시도 ${attempt + 1}/${MAX_ATTEMPTS} 실패: ${err.message}`,
      );
    }
  }
  console.warn(
    `⚠️ buildCardIntroSummary[${(card.trend_name ?? "?").slice(0, 25)}]: 임계 초과 — fallback`,
  );
  return null;
}
...
function buildIntroBodyFallback(td, rank) {
  ...
  // 기간(metrics.period) — 한국어로 변환해 출처 뒤에 자연스럽게 삽입.
  const periodKo = formatPeriodKo(td?.metrics?.period);
  const periodTag = periodKo ? `${periodKo} ` : "";
...
function buildStatSentence(td, idx) {
  // 어미 통일 — 3행 모두 "있어요"로 끝나도록 통일.
  // 0행 "부상 중이에요" → "떠오르고 있어요" (부상 동음이의 회피 + 어미 통일)
  // 1행 "확산되고 있어요"·"자리잡은 트렌드예요" → 모두 "있어요"로 통일
  // 2행 "모이고 있어요" (기존 그대로)
  if (idx === 0) {
    const score = td?.metrics?.score;
    if (typeof score !== "number") return null;
    ...
    return `${sourcePrefix}검색 관심도가 ${tier} 수준으로 꾸준히 떠오르고 있어요.`;
  }
  if (idx === 1) {
    const rate = td?.metrics?.growth_rate;
    ...
    return pct > 0
      ...
      : `${prefix}최근 ${abs}% 감소했지만 안정적인 흐름을 보이고 있어요.`;
  }
  if (idx === 2) {
    const df = td?.demand_fit;
    if (!df || df.invalid_keyword === true) return null;
    const ms = df.monthly_searches;
    if (typeof ms !== "number" || ms <= 0) return null;
    return `네이버 검색광고 기준 월 ${ms.toLocaleString("ko-KR")}건 수준의 관심이 모이고 있어요.`;
  }
  return null;
}

// 매칭가의 분석체 어미("핵심 매칭", "부합", "일치" 같은 명사 종결)를 stat 문장과
// 같은 해요체로 통일. 한 문장 안에 분석체 + 해요체 섞임 인상 제거.
//   "핵심 매칭" → "핵심적으로 매칭돼요"
//   "부합"      → "부합해요"
//   "일치"      → "일치해요"
// 이미 어미가 있으면(요/예요/이에요/입니다/있어요/돼요/해요 등) 변형 안 함.
function normalizePolishedEnding(text) {
  if (!text) return text;
  const t = String(text).trim().replace(/[.。!?…]+\s*$/, "");
  // 어미가 이미 있는지 확인 — "있어요/돼요/해요/예요/이에요" 등
  if (/요$|예$|니다$|어요$|아요$|돼요$|해요$|이에요$|예요$/.test(t)) return t;
  // 동사성 명사 종결 → 동사형으로 변환. 부사 capture는 trailing 공백 보정 위해
  // 별도 그룹으로 받지 않고 분기 패턴으로 처리.
  const VERB_FORM = [
    [/핵심\s*매칭$/, "핵심적으로 매칭돼요"],
    [/매칭$/, "매칭돼요"],
    [/부합$/, "부합해요"],
  ...
  for (const [pattern, replacement] of VERB_FORM) {
    if (pattern.test(t)) return t.replace(pattern, replacement);
  }
  // 폴백: 마지막 글자 받침에 따라 "이에요/예요" 자동 부착
  const lastChar = t[t.length - 1];
  if (!lastChar) return t;
  const code = lastChar.charCodeAt(0);
  if (code >= 0xac00 && code <= 0xd7a3) {
    const hasBatchim = (code - 0xac00) % 28 !== 0;
    return t + (hasBatchim ? "이에요" : "예요");
  }
  return t;
}
...
// naturalizeMatcherText()가 자연 한글로 치환. polished(매칭가 reason)를 먼저 두고
// 끝에 fitIndex별 stat 문장을 붙여, 카드 안 3항목이 같은 prefix로 보이지 않게 한다.
// polished는 normalizePolishedEnding으로 해요체로 변환 → stat 문장과 어미 통일.
function buildMatchReason(fitData, td, fitIndex = 0) {
  ...
  const stat = buildStatSentence(td, fitIndex);
  if (!polished) return stat ?? "";
  // 분석체 → 해요체 변환 + 끝에 마침표 보장.
  const normalized = normalizePolishedEnding(polished);
  const polishedFinal = /[.。!?]$/.test(normalized) ? normalized : `${normalized}.`;
  return stat ? `${polishedFinal} ${stat}` : polishedFinal;
}
...
export async function generateWriterOutput({ brand, trend, match, trendRaw } = {}) {
  ...
  const dedicatedReasons = rawContents.map((c) => {
    ...
    return Object.fromEntries(
      FIT_KEYS.map((key, idx) => [key, buildMatchReason(c.raw_fits?.[key], td, idx)]),
    );
  });
  ...
  const contents = rawContents.map((c, i) => {
    ...
    // summary_bullets 끝에 통계 한 줄 요약 추가 — 카드별 stat 반복 없이
    // 트렌드 레벨 수치(검색 관심도·증가율·월간 검색량)를 한 곳에 모음.
    // LLM 재작성 단계 이후라 원문 그대로 유지된다.
    const baseSummaryBullets =
      Array.isArray(enr?.summary_bullets) && enr.summary_bullets.length > 0
        ? enr.summary_bullets
        : c.summary_bullets;
    const statsLine = buildStatsSummaryLine(td);
  ...
  // 4) 카드별 PART I 인트로 요약 — 트렌드현황과 똑같은 텍스트 반복 방지.
  //    각 카드 LLM 1회. 유사도 < 10% 가드 + 5회 재시도, 실패 시 intro_summary 누락
  //    → 렌더러는 summary_bullets[0]로 fallback.
  const cardIntros = await Promise.all(
    contents.map((c) => buildCardIntroSummary(c, client)),
  );
  contents.forEach((c, i) => {
    if (cardIntros[i]) c.intro_summary = cardIntros[i];
  });
  ...
  // 트렌드 한 문장 요약 — LLM이 contents 3개 카드를 종합. 성공해도 contents는
  // 그대로 유지 (Part II 트렌드현황·수집근거·매칭이유 렌더링에 필요).
  // trend_summary는 Part I(브리프) 자리를 한 문장으로 압축하는 용도.
  const trendSummary = await buildTrendSummary({ contents, client });
  ...
  if (trendSummary) {
    dataPayload.trend_summary = trendSummary;
  }
...
if (isDirectRun) {
  ...
  // 1) JSON 구조화 산출물 (UI 서빙용) — 팀 합의로 output-main/output-text/에 저장하고
  ...
  // 2) 마크다운 리포트 (사람용) — JSON 산출 시 만들어진 trend_summary를 전달.
  //    요약이 있으면 카드 영역이 한 문장으로 대체, 없으면 기존 카드 fallback.
  const trendSummary = writerJson?.data?.trend_summary ?? null;
  const md = generateReport({ brand, trend, match, trendSummary });
