...
// ─── match_fits.casual_bullets 전용 LLM (영역별 한 카드 1회씩) ───────
// 매칭가의 reason을 비전공자도 이해 가능한 구어체 불렛으로 풀어 씀.
// 요약·축소 금지 — 매칭가 원문의 모든 내용을 살리되 표현만 재작성.
//
// 입력: 영역별 매칭가 reason (이미 naturalizeMatcherText로 자연 한글화됨)
// 출력: 영역별 구어체 불렛 배열 (보통 2~4개)
const CasualBulletsLlmSchema = z.object({
  ingred: z.array(z.string().min(5)).min(2).max(5),
  visual: z.array(z.string().min(5)).min(2).max(5),
  life: z.array(z.string().min(5)).min(2).max(5),
  safe: z.array(z.string().min(5)).min(2).max(5),
});

const CASUAL_BULLETS_SYSTEM_PROMPT = `당신은 매칭가의 평가 문장을 비전공자가 이해 가능한 구어체 불렛으로 풀어 쓰는 작업가입니다.

핵심 원칙 (지키지 않으면 작업 실패):
1) **요약·축소·생략 절대 금지** — 입력 매칭가 문장의 모든 정보를 빠짐없이 살려야 함.
2) **매칭가 원문 그대로 복사 금지** — 비전공자가 읽기 좋게 표현을 풀어 쓸 것.
3) **환각 금지** — 입력에 없는 사실·수치·키워드를 만들어내지 말 것. 단어 풀어 쓰기만 가능.
4) **구어체** — "~합니다", "~으로 판정" 같은 격식체 → "~예요", "~이거든요", "~하답니다" 같은 자연체로.
5) 매칭 결과(✅/⚠️/❌) 톤을 자연스럽게 반영:
   - ✅: "잘 맞아요", "딱이에요" 등 긍정 톤
   - ⚠️: "조금 부분적이지만", "주의는 필요해요" 등 조건부 톤
   - ❌: "안 맞아요", "충돌해요" 등 부정 톤
6) 영역마다 불렛 2~4개. 한 불렛은 한 정보 단위. 마지막 불렛은 결과·결론 톤.

각 영역(ingred·visual·life·safe)마다 한 배열을 채워서 JSON으로만 반환.`;

async function generateCasualVerdictBullets({ rawContent, td, client }) {
  if (!client) return null;
  const fits = rawContent?.match_fits ?? {};
  // 4영역 모두 reason이 비어있으면 LLM 호출 스킵
  if (!fits.ingred?.reason && !fits.visual?.reason && !fits.life?.reason && !fits.safe?.reason) {
    return null;
  }

  const userMessage = `## 트렌드
- 이름: ${td?.trend_name ?? "-"}
- 검색량: ${td?.headline_metric?.value ?? "-"} (${td?.headline_metric?.delta ?? "-"})

## 4영역 매칭가 평가 (이 모든 내용을 빠짐없이 풀어 써주세요)

### 제품 적합도 (ingred) — 결과: ${fits.ingred?.result ?? "-"}
${fits.ingred?.reason ?? "(없음)"}

### 비주얼·톤 적합도 (visual) — 결과: ${fits.visual?.result ?? "-"}
${fits.visual?.reason ?? "(없음)"}

### 타겟 적합도 (life) — 결과: ${fits.life?.result ?? "-"}
${fits.life?.reason ?? "(없음)"}

### 안전성·지속성 (safe) — 결과: ${fits.safe?.result ?? "-"}
${fits.safe?.reason ?? "(없음)"}

각 영역의 모든 정보를 살려 구어체 불렛으로 풀어 쓴 뒤 JSON으로만 반환.`;
  ...
  try {
    const response = await client.messages.parse({
      ...
      temperature: 0.3,
      system: [
        { type: "text", text: CASUAL_BULLETS_SYSTEM_PROMPT, cache_control: { type: "ephemeral" } },
      ],
      messages: [{ role: "user", content: userMessage }],
      output_config: { format: zodOutputFormat(CasualBulletsLlmSchema) },
    });
    return response.parsed_output ?? null;
  } catch (err) {
    console.warn(`⚠️ generateCasualVerdictBullets 실패 (${td?.trend_name ?? "?"}): ${err.message}`);
    return null;
  }
}
...
export async function generateWriterOutput({ brand, trend, match } = {}) {
  ...
  // 2-3) match_fits.casual_bullets — 매칭가 reason을 비전공자 친화 구어체 불렛으로
  //      재작성. 카드 1장당 LLM 1회 호출 (4영역 동시 처리). 요약·축소 금지.
  //      raw matchReasons (이미 buildMatchReason 거친 값)를 LLM 입력으로 전달.
  const casualBulletsByCard = await Promise.all(
    rawContents.map((c) => {
      const td = findTrend(c.trend_name);
      // matchReasons[i]를 임시로 match_fits에 주입한 가짜 rawContent로 LLM 호출
      const idx = rawContents.indexOf(c);
      const augmentedFits = Object.fromEntries(
        FIT_KEYS.map((k) => [k, {
          ...(c.match_fits?.[k] ?? {}),
          reason: matchReasons[idx][k] || c.match_fits?.[k]?.reason || "",
        }]),
      );
      return generateCasualVerdictBullets({
        rawContent: { ...c, match_fits: augmentedFits },
        td,
        client,
      });
    }),
